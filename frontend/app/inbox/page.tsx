...
export default function InboxPage() {
  ...
  return (
    <AppShell>
      <div className="relative flex h-[calc(100vh-64px)] overflow-hidden rounded-2xl border border-indigo-100/70 bg-white/60 backdrop-blur-sm">
        ...
        <div className={`
          flex min-h-0 flex-1 flex-col overflow-hidden border-r border-indigo-100 bg-white/80 lg:w-80 lg:flex-none
          ${mobileViewingEmail ? 'hidden lg:flex' : 'flex'}
        `}>
          {/* Mobile Header with Menu Button */}
          <div className="flex shrink-0 items-center gap-2 border-b border-indigo-100 p-2 sm:p-3 lg:hidden">
            <button
              onClick={() => setShowMobileSidebar(true)}
              className="shrink-0 rounded-xl p-2 hover:bg-indigo-50"
            >
            ...
            <div className="flex-1 relative min-w-0">
              ...
              <Input
                ...
                className="h-9 w-full bg-white/90 pl-9 text-sm placeholder:text-slate-400"
              />
            </div>
          </div>

          {/* Desktop Search */}
          <div className="hidden shrink-0 border-b border-indigo-100 p-3 lg:block">
            <div className="relative">
              ...
              <Input
                ...
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="h-9 bg-white/90 pl-9 text-sm placeholder:text-slate-400"
              />
            </div>
          </div>

          {/* List Header */}
          <div className="shrink-0 border-b border-indigo-100 px-3 py-2 sm:px-4">
            <h2 className="text-sm font-medium capitalize text-slate-900">{activeTab}</h2>
            <p className="text-xs text-slate-400">{filteredEmails.length} emails</p>
          </div>
        ...
        <div className={`
          ...
          ${mobileViewingEmail ? 'fixed inset-0 top-14 z-30 bg-white/95 lg:relative lg:top-0 lg:z-auto' : ''}
        `}>
          <EmailViewer
            email={selectedEmail}
            showReply={showReply}
