  ...
  if (error) {
    return (
      <div className="p-6 text-center">
        ...
        <p className="mb-1 text-sm font-medium text-slate-700">Failed to load emails</p>
    ...
    return (
      <div className="p-8 text-center">
        <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-indigo-100">
          <MailOpen className="h-8 w-8 text-indigo-400" />
        </div>
        <p className="text-sm font-medium text-slate-700">No emails here</p>
        ...
        return (
          <div
            ...
            className={`
              ...
              ${isSelected 
                ? 'border-l-2 border-l-indigo-500 bg-indigo-50/70'
                : 'border-l-2 border-l-transparent hover:bg-indigo-50/50'
              }
              ${!email.isRead ? 'bg-white/90' : 'bg-slate-50/60'}
            `}
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between gap-2 mb-0.5">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    {isHovered && (onArchive || onTrash) ? (
                      <div className="flex items-center gap-0.5">
                        {onArchive && (
                          <button
                            ...
                            className="rounded-full p-1.5 text-slate-400 transition-colors hover:bg-indigo-100 hover:text-indigo-700"
                            title="Archive"
                          >
                      ...
                      <span className={`text-xs ${!email.isRead ? 'font-medium text-slate-900' : 'text-slate-500'}`}>
                ...
                <p
                  className={`text-sm truncate ${
                    !email.isRead ? 'font-medium text-slate-900' : 'text-slate-700'
                  }`}
                >
                ...
                <div className="flex items-center gap-1 sm:gap-1.5 mt-1 sm:mt-1.5 flex-wrap">
                  ...
                  {email.labels?.includes('ATTACHMENT') && (
                    <span className="inline-flex items-center gap-0.5 rounded-full bg-slate-100 px-1.5 py-0.5 text-[10px] text-slate-500">
            ...
            {!email.isRead && (
              <div className="absolute left-1 top-1/2 h-2 w-2 -translate-y-1/2 rounded-full bg-indigo-500" />
            )}
          </div>
        );
      })}
